...
class AnalogIo:
    ...
    class Channel:
        ...
        class Node:
            ...
            def value_min(self) -> float:
                ...
                return api.dwf_analog_io_channel_node_set_info(
                    self._device.handle, self._channel.index, self._node)[0]

            @property
            def value_max(self) -> float:
                """Gets the maximum node value."""
                return api.dwf_analog_io_channel_node_set_info(
                    self._device.handle, self._channel.index, self._node)[1]

            @property
            def value_steps(self) -> int:
                """Gets the number of node value steps."""
                return api.dwf_analog_io_channel_node_set_info(
                    self._device.handle, self._channel.index, self._node)[2]
        ...
        def __init__(self, module, channel):
            ...
            self._module = module
            self._channel = channel
            self._nodes = tuple(self.Node(self, i) for i in range(
                api.dwf_analog_io_channel_info(self._device.handle, self._channel)))
    ...
    def __init__(self, device):
        ...
        self._channels = tuple(self.Channel(self, i) for i in range(
            api.dwf_analog_io_channel_count(self._device.handle)))
